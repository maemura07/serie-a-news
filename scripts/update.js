 ...
 *  2. URLベースで重複排除（既に処理済みの記事はスキップ）
 *  3. 新着記事を Claude API でまとめて判定・日本語要約
 *     （対象クラブ・対象トピックに関係ない記事は捨てる）
 *  4. data/articles.json に保存 → 静的ページが読み込む
 *
...
const BATCH_SIZE = 8;            // 1回のAPI呼び出しでまとめて処理する記事数
const MAX_ARTICLES_KEPT = 300;   // ページに保持する記事数の上限
const MAX_SEEN_URLS = 5000;      // 重複排除用に覚えておくURL数の上限
...
【翻訳・要約ルール】
- headline_ja: 日本語の見出し（30字以内、体言止め可）
- summary_ja: 見出しとリード文の内容を、自然な日本語2〜3文にまとめる。無理に言い換えず、原文の意味に忠実な翻訳調で構わない。ただし翻訳調すぎて不自然な日本語にならないこと（自然さ最優先）。
- 選手名・監督名は日本のサッカーメディアで一般的なカタカナ表記を使う。
- 情報を捏造しない。見出しとリード文から分かる範囲だけで書く。
...
// ====== メイン ======
async function main() {
  const sourcesConf = loadJson(SOURCES_FILE, { sources: [] });
  const store = loadJson(DATA_FILE, { updated: null, articles: [], seenUrls: [] });
  const seen = new Set(store.seenUrls || []);
  // 既掲載記事のURLも重複排除対象に含める
  (store.articles || []).forEach((a) => seen.add(normalizeUrl(a.url)));

  const parser = new Parser({ timeout: 20000 });
  const candidates = [];
  const feedStatus = [];

  for (const src of sourcesConf.sources || []) {
    ...
    try {
      ...
      feedStatus.push({ name: src.name, ok: true, newItems: count });
      console.log(`OK: ${src.name} — 新着候補 ${count} 件`);
    } catch (e) {
  ...
  for (let i = 0; i < targets.length; i += BATCH_SIZE) {
      ...
      for (const r of results) {
        ...
        if (r.relevant && r.headline_ja && r.summary_ja) {
          accepted.push({
            ...
            headline: String(r.headline_ja).slice(0, 60),
            summary: String(r.summary_ja).slice(0, 500),
    ...
    // 処理を試みたURLは成否に関わらず「見た」ことにする（無限リトライ防止）
  ...
  const newStore = {
    ...
    feedStatus,
    articles,
    seenUrls: [...seen].slice(-MAX_SEEN_URLS),
  };
